...
class Application:
    ...
    def update_labelframes(self, variant):
        if self.have_internet():
            if variant == "S_Regular_Highwind":
                ...
                self.set_content_to_labelframes_labels("S_Very_Strict_Highwind", "https://raw.githubusercontent.com/ffhighwind/PoE-Price-Lister/master/Resources/Filters/S_Very_Strict_Highwind.filter", self.highwind_very_strict_size_label, self.highwind_very_strict_last_mod_label)
            else:
    ...
    def download_highwind_filter(self, variant):
        ...
        if variant == "S_Regular_Highwind":
            ...
            my_logger.error("download_highwind_filter method. Unknown variant: " + variant)
